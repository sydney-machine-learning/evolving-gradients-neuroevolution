...
class neuralnetwork:
	...
	def __init__(self, Topo, Train, Test, learn_rate):
		...
		self.Gw1 = np.random.randn(self.Top[1]) / np.sqrt(self.Top[1])

		self.B1 = np.random.randn(1, self.Top[1]) / np.sqrt(self.Top[1])  # bias first layer
		self.W2 = np.random.randn(self.Top[1], self.Top[2]) / np.sqrt(self.Top[1])
		self.Gw2 = np.random.randn(self.Top[2]) / np.sqrt(self.Top[2])
	...
	def BackwardPass_evolve(self, Input): # since data outputs and number of output neuons have different orgnisation
		out_delta = self.Gw2
		hid_delta = self.Gw1
		self.W2 += np.dot(self.hidout.T,(out_delta * self.lrate))
		self.B2 += (-1 * self.lrate * out_delta)
	...
	def decode_gradients(self, g):
		self.Gw1 = g[:self.Top[1]]
		self.Gw2 = g[self.Top[1]:self.Top[2]]
	
	def evolvegradients(self, data, g):  # BP with SGD (Stocastic BP)

		self.decode_gradients(g)
		size = data.shape[0]
		...
		for i in range(0, size):
			...
			# Desired = data[pat, self.Top[0]:]
			self.ForwardPass(Input)
			self.BackwardPass_evolve(Input)
		w_updated = self.encode()
...
class neuroevolution(object):  # class for fitness func 
	def __init__(self,  netw, traindata, testdata):
		...
		self.temp_neural_net = copy.deepcopy(self.neural_net)
	...
	def fit_func(self, x):    #  function  (can be any other function, model or diff neural network models (FNN or RNN ))
		...
		if self.problem == 1: # rosenbrock
			...
			fx, prob = self.temp_neural_net.evaluate_proposal(self.traindata,x)
			fit= self.rmse(fx,y) 
	...
	def evolve_gradient(self, data, g, save=False):  # BP with SGD (Stocastic BP)

		if save==True:
			weights = self.neural_net.evolvegradients(data, g)
		else:	

			weights = self.temp_neural_net.evolvegradients(data, g)
...
class evolutionPSO(neuroevolution):  #G3-PCX Evolutionary Alg by K Deb - 2002 
	...
	def evolvePSO(self):
		...
		while epoch < self.max_epochs:
			...
			best_epoch_error = sys.float_info.max
			best_epoch_pos = [0.0 for i in range(self.dim)]
			...
			for i in range(self.n): # process each particle
				...
				if evolve_gradients == True: 

					updated_weights = self.evolve_gradient(self.traindata, swarm[i].position.copy())  
					
 
				swarm[i].error = self.fit_func(updated_weights)
				...
				if swarm[i].error < best_epoch_error:
					best_epoch_error = swarm[i].error
					best_epoch_pos = copy.copy(swarm[i].position)

			if evolve_gradients == True: 

				best_weights = self.evolve_gradient(self.traindata, best_epoch_pos, save=True)

			

			if epoch % 10 == 0 and epoch > 1:
				print("Epoch = " + str(epoch) + " best error = %.7f" % best_epoch_error)

				train_per, rmse_train = self.classification_perf(best_weights, 'train')
				test_per, rmse_test = self.classification_perf(best_weights, 'test')
...
def main():
	...
	for problem in range(1, 9) : 
		...
		num_varibles = (netw[1] + netw[2])  # num of weights and bias
